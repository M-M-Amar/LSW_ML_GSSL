...
class Model(torch.nn.Module):
    def __init__(self, encoder: Encoder, num_hidden: int, num_proj_hidden: int, edges, num_nodes, device,
                 tau: float = 0.5, m: float = 0.15, gamma: float = 1.5, batch: int = 0, num_clust: int = 7, walk_length: int = 3):
        ...
        self.walk, self.non_neighbors = self.rd_walk()
    ...
    def rd_walk(self):
        ...
        self_loops = torch.stack([torch.arange(self.num_nodes, device=self.device), torch.arange(self.num_nodes, device=self.device)], dim=0)
        ...
        walk = walk[:, 1:]
        return walk, non_neighbors

    def random_walks_mean(self, z: torch.Tensor, walk, non_neighbors) -> torch.Tensor:
        # znp = z.detach().numpy()
    ...
    def proximity(self, z: torch.Tensor) -> torch.Tensor:
        return self.random_walks_mean(z, self.walk, self.non_neighbors)
    ...
    def semi_loss(self, z1: torch.Tensor, z2: torch.Tensor, z3: torch.Tensor) -> torch.Tensor:
        ...
        if self.epch>200 and self.epch%10==0:
        ...
        if self.epch>200 and self.epch%10==0:
            ll = torch.log(1 + torch.exp(self.gamma * (an_1 * (Sn_1 - delta_n) - ap_1 * (Sp_1 - delta_p) + an_3 * (Sn_3 - delta_n) - ap_3 * (Sp_3 - delta_p) + an_2 * (Sn_2 - delta_n) - ap_2 * (Sp_2 - delta_p) + an_4 * (Sn_4 - delta_n) - ap_4 * (Sp_4 - delta_p))))
        else:
            ll = torch.log(1 + torch.exp(self.gamma * (an_1 * (Sn_1 - delta_n) - ap_1 * (Sp_1 - delta_p) + an_3 * (Sn_3 - delta_n) - ap_3 * (Sp_3 - delta_p)+ an_4 * (Sn_4 - delta_n) - ap_4 * (Sp_4 - delta_p))))
...
def train_and_extract_ID_LID_multi_level(
    ...
    model = Model(encoder, num_hidden, num_proj_hidden, data.edge_index, len(data.x), device, tau=tau, batch=batch, num_clust=num_classes, m=m, gamma=gamma).to(device)
