...
class Model(torch.nn.Module):
    ...
    def semi_loss(self, z1: torch.Tensor, z2: torch.Tensor, z3: torch.Tensor) -> torch.Tensor:
        ...
        mask_tensor = mask_tensor.to(device)
        prox = z2
        Sp_2 = torch.mm(z1, prox.t()).diag()
        Sn_2 = torch.mul(torch.mm(z1, prox.t()), mask_tensor).sum(1)
        ...
        ll = torch.log(1 + torch.exp(self.gamma * (an_1 * (Sn_1 - delta_n) - ap_1 * (Sp_1 - delta_p) + an_2 * (Sn_2 - delta_n) - ap_2 * (Sp_2 - delta_p) + an_3 * (Sn_3 - delta_n) - ap_3 * (Sp_3 - delta_p))))
        return ll
...
if not args.pretrained:
    print(
        f'{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")} {args.dataset}, Multi-level, Epoch:{best_epoch}, AUC-mean: {np.mean(auc_scores) * 100}')
else:
    print(
